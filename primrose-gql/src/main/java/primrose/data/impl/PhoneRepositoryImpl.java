...
public class PhoneRepositoryImpl implements PhoneRepository {
  ...
  public List<Phone> customerPhones(long customerId) {
    return create
      ...
      .leftJoin(Tables.PHONE_NUMBER_TYPES).on(Tables.PHONE_NUMBER_TYPES.ID.eq(Tables.CUSTOMER_PHONE_NUMBERS.PHONE_NUMBER_TYPE))
      .where(Tables.CUSTOMER_PHONE_NUMBERS.CUSTOMER.eq(customerId))
      .fetch()
      .map(record -> new Phone(record.value1(), record.value2(), record.value3()));
  }
  ...
  public List<Phone> contactPhones(long contactId) {
    return create
      ...
      .leftJoin(Tables.PHONE_NUMBER_TYPES).on(Tables.PHONE_NUMBER_TYPES.ID.eq(Tables.CONTACT_PHONE_NUMBERS.PHONE_NUMBER_TYPE))
