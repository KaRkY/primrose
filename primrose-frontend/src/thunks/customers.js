...
import loadCustomers from "../api/loadCustomers";
...
export default async (dispatch, getState) => {
  ...
  if(isCustomersLoading(getState())) return;
  ...
  loadCustomers({
    ...
    .then(result => dispatch({ type: "CUSTOMERS_FETCHED", payload: result }))
    .catch(error => dispatch({ type: "CUSTOMERS_ERROR" }));
};