...
@Component
public class AccountContactTypeResourceAssembler
  extends ResourceAssemblerSupport<AccountContactType, AccountContactTypeResource> {
  ...
  public AccountContactTypeResource toResource(final AccountContactType domain) {
    return ImmutableAccountContactTypeResource.builder()
      ...
      .putLink("self", ImmutableLink.builder()
        .href(fromController(AccountContactTypesController.class).path("/{type}").buildAndExpand(domain.name())
          .toUriString())
        .title(domain.name())
        .build())
      .build();
